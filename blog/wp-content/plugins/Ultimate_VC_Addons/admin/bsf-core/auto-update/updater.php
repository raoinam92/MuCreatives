...
if(!function_exists('bsf_get_remote_version')) {
	function bsf_get_remote_version($products , $get_from_dt = false ){
...
if ( ! function_exists( 'bsf_check_product_update' ) ) {
	function bsf_check_product_update() {
		...
		$dt_remote_versions = bsf_get_remote_version($registered, true);
		...
		foreach ( $bsf_product_plugins as $id => $product ) {
